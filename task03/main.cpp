    ...
    for (unsigned int jx = 0; jx < num_div; ++jx) {
      for (unsigned int jy = 0; jy < num_div; ++jy) {
        if (abs_diff(ix, jx) <= 1 && abs_diff(iy, jy) <= 1) { // this grid is near to the particle as the grid indexes are close
          ...
          int nums = acc.grid2idx[jy * num_div + jx + 1] - acc.grid2idx[jy * num_div + jx]; // #particles for each gird
          particles[ip].force += (gravitational_force(acc.grid2cg[jy * num_div + jx] - particles[ip].pos) * nums);
        }
      }
    }
  }
