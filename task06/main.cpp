...
void WdWddW_Spring3(
  ...
  const Eigen::Vector3d delta_p = (node2xyz[0] - node2xyz[1]);
  const Eigen::Matrix3d outerProdOfPosi = delta_p * delta_p.transpose();
  const Eigen::Matrix3d H = (Identity * stiffness) - (Identity * (stiffness * length_ini / length)) + outerProdOfPosi * (stiffness * length_ini / (length * length * length));
  ddw[0][0] = H;
  ddw[0][1] = -H;
  ddw[1][0] = -H;
  ddw[1][1] = H;
