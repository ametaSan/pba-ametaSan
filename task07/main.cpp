...
void solve_laplace_gauss_sidel_on_grid(
  ...
  for (unsigned int ix = 0; ix < grid_size; ++ix) {
    for (unsigned int iy = 0; iy < grid_size; ++iy) {
      ...
      if((ix>0)&&(ix<grid_size-1)&&(iy>0)&&(iy<grid_size-1))
        vtx2val[idx_center] = (vtx2val[idx_center-1]+vtx2val[idx_center+1]+vtx2val[idx_center-grid_size]+vtx2val[idx_center+grid_size])/4;
    }
  }
}
